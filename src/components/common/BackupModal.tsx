...
export const BackupModal: React.FC<BackupModalProps> = ({ isOpen, onClose }) => {
  ...
  const generateBackupPayload = () => {
    ...
    if (scopeMode === 'today') {
      ...
      return {
        ...
        summary: {
          ...
          totalRecords: todayReports.length + todayPayments.length + todayExpenses.length + todayLeads.length + todayMessages.length + todayLogs.length
        },
        data: {
    ...
    if (scopeMode === 'all') {
      return {
        ...
        summary: {
          ...
          totalRecords: users.length + courses.length + projects.length + dailyReports.length + customerLeads.length + tuitions.length + payments.length + expenses.length + teacherSalaries.length + messages.length + notifications.length + activityLogs.length
        },
        data: {
          users,
          courses,
    ...
    if (selectedModules.financial) {
      ...
      customSummary.financialRecordsCount = tuitions.length + payments.length + expenses.length + teacherSalaries.length;
    ...
    return (
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
        <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-6 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-4 text-center relative">
          ...
          <h3 className="font-extrabold text-base text-slate-900 dark:text-white">دسترسی غیرمجاز</h3>
          <p className="text-xs text-slate-500">قابلیت پشتیبان‌گیری و ارسال ایمیل اطلاعات، منحصراً در اختیار مدیر کل آموزشگاه می‌باشد.</p>
          <button
            onClick={onClose}
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto animate-fade-in">
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-2xl w-full p-6 sm:p-7 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-6 my-8 max-h-[90vh] overflow-y-auto animate-scale-up">
        ...
        {/* Alternative Export Options: Direct Download & Copy */}
        <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              ...
              className="flex-1 sm:flex-none px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl shadow-md flex items-center justify-center gap-1.5 transition-all"
            >
