...
import { getTodayPersianDate, calculateCourseSessionsTimeline, MonthlySessionsBreakdown } from '../../utils/dateUtils';
...
export const CoursesModule: React.FC = () => {
  ...
  const [isAiAnalyzingSchedule, setIsAiAnalyzingSchedule] = useState(false);
  const [aiScheduleSummary, setAiScheduleSummary] = useState('');
  ...
  const handleOpenModal = (course?: Course) => {
    ...
    if (course) {
      ...
      setCustomDatesInput('');
      setStartDate(getTodayPersianDate());
      setEndDate(`${getTodayPersianDate().split('/')[0] || '1405'}/12/29`);
  ...
  const handleTriggerAiScheduleAnalysis = async () => {
    setIsAiAnalyzingSchedule(true);
    try {
      const res = await fetch('/api/gemini/analyze-course-schedule', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          courseTitle: title || 'دوره آموزشی',
          category,
          startDate,
          endDate,
          scheduleDays: isScheduleCustomMode ? 'روزها و تاریخ‌های سفارشی' : scheduleDays,
          customDays,
          customDates: currentParsedCustomDates,
          holidaysContext: currentScheduleAnalysis.monthlyBreakdown
        })
      });
      const data = await res.json();
      if (data.success && data.data?.scheduleSummary) {
        setAiScheduleSummary(data.data.scheduleSummary);
      } else {
        setAiScheduleSummary(`تحلیل هوشمند تقویم: در بازه انتخابی مجموعاً ${currentScheduleAnalysis.totalSessions} جلسه در ${currentScheduleAnalysis.monthlyBreakdown.length} ماه بدون تداخل با تعطیلات رسمی برگزار خواهد شد.`);
      }
    } catch (e) {
      setAiScheduleSummary(`تحلیل تقویم: مجموعاً ${currentScheduleAnalysis.totalSessions} جلسه در ${currentScheduleAnalysis.monthlyBreakdown.length} ماه محاسبه شد.`);
    } finally {
      setIsAiAnalyzingSchedule(false);
    }
  };
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-lg w-full p-6 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-4 animate-scale-up">
            ...
            <form onSubmit={handleSaveCourse} className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label className="text-slate-600 dark:text-slate-300 font-semibold">مدرس دوره</label>
                    <button
                      type="button"
                      onClick={() => setUseManualTeacher(!useManualTeacher)}
                      className="text-[10px] text-blue-600 dark:text-blue-400 font-bold underline"
                    >
                      {useManualTeacher ? 'انتخاب از لیست اساتید' : 'ورود دستی نام مدرس'}
                    </button>
                  </div>
                  ...
                  {useManualTeacher ? (
                    <input
                      type="text"
                      placeholder="نام و نام خانوادگی استاد را بنویسید"
                      value={manualTeacherName}
                      onChange={e => setManualTeacherName(e.target.value)}
                      required
                      className="w-full bg-slate-50 dark:bg-slate-800 border border-blue-400 rounded-xl p-2.5 outline-none text-slate-800 dark:text-white font-bold"
                    />
                  ) : (
                    <select
                      value={teacherId}
              ...
              {/* Schedule Dates & Days Section with Persian Calendar & AI Analysis */}
              <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                    ...
                    بازه زمانی برگزاری و تحلیل هوشمند تقویم
                  </span>
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                    ...
                    >
                      {isScheduleCustomMode ? '✓ روزها / تاریخ‌های دلخواه' : '+ تعیین تاریخ‌ها/روزهای سفارشی'}
                    </button>
                    <button
                      type="button"
                      onClick={handleTriggerAiScheduleAnalysis}
                      disabled={isAiAnalyzingSchedule}
                      className="px-2.5 py-1 rounded-lg text-[11px] font-bold bg-purple-600 hover:bg-purple-500 text-white flex items-center gap-1 shadow-sm disabled:opacity-50"
                    >
                      <Sparkles className="w-3.5 h-3.5" />
                      {isAiAnalyzingSchedule ? 'در حال تحلیل...' : 'تحلیل هوش مصنوعی'}
                    </button>
                  </div>
                </div>
                ...
                <div className="p-3 rounded-xl bg-blue-50/70 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/80 space-y-2">
                  <div className="flex flex-wrap items-center justify-between text-xs gap-2">
                    <span className="font-bold text-blue-950 dark:text-blue-200">
                      📊 محاسبه تقویمی: مجموع جلسات مفید: <strong className="text-blue-600 dark:text-blue-400 text-sm font-black">{currentScheduleAnalysis.totalSessions} جلسه</strong>
                    </span>
                  ...
                  {aiScheduleSummary && (
                    <div className="pt-2 border-t border-blue-200/60 dark:border-blue-800/60 text-[11px] text-purple-900 dark:text-purple-200 font-medium leading-relaxed">
                      🤖 <strong>تحلیل هوش مصنوعی:</strong> {aiScheduleSummary}
                    </div>
                  )}
                </div>
              </div>
