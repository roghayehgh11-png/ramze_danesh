  ...
  const handlePdfUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (extracted) {
        ...
        setTomorrowsPlan(extracted.tomorrowsPlan || '');
        if (extracted.progressAdded) setProgressAdded(extracted.progressAdded);

        setAiSuccessMessage(
          `✨ فایل ضمیمه توسط هوش مصنوعی بررسی شد و عیناً مطابق محتوای درج‌شده استخراج گردید.`
        );
      }
    } catch (err) {
    ...
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {(currentUser.role === 'intern' || currentUser.role === 'admin') && (
            <button
      ...
      {/* MODAL: ADD DAILY REPORT WITH PDF AI AUTO-FILL */}
      {isAddModalOpen && (
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-xl w-full p-6 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-4 animate-scale-up my-8">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <h3 className="font-bold text-base text-slate-900 dark:text-white flex items-center gap-2">
                ...
                ثبت گزارش کار روزانه (با هوش مصنوعی)
              </h3>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveReport} className="space-y-3.5 text-xs">
              ...
              {currentUser.role === 'admin' ? (
                ...
                <div className="p-2.5 bg-blue-50/60 dark:bg-blue-950/40 rounded-xl border border-blue-200 text-blue-900 dark:text-blue-200 font-bold flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-blue-600" />
                  <span>کارآموز: {currentUser.name}</span>
                </div>
              )}
              ...
              <div>
                ...
                <textarea
                  ...
                  required
                  placeholder="برنامه‌ریزی شما برای فردا (مثلاً تکمیل پیاده‌سازی فرم و تست نهایی)..."
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-2.5 outline-none text-slate-800 dark:text-white"
                ></textarea>
              </div>
